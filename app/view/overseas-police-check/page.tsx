...
import {
  ChevronDown, ChevronUp, Check, X, Shield, User, FileText,
  Calendar, AlertTriangle, Eye, ArrowLeft, Code, CheckCircle
} from 'lucide-react';
...
export default function OverseasPoliceCheckViewPage() {
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(
    new Set(['pre_validation', 'personal_information', 'validity_status'])
  );
  ...
  // Special renderer for security_assessment with security_features array
  const SecurityAssessmentSection = ({
    data,
  }: {
    data: Record<string, unknown>;
  }) => {
    const isExpanded = expandedSections.has('security_assessment');
    const hasData = data && Object.keys(data).length > 0;
    ...
    return (
      <div className="bg-red-50 rounded-xl shadow-sm border border-red-200 overflow-hidden">
        <button
          className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors"
          onClick={() => toggleSection('security_assessment')}
        >
          <div className="flex items-center gap-3">
            ...
            {hasData ? (
              <div className="space-y-4">
                {Object.entries(data).map(([key, valueObj]) => {
                  ...
                  // Special handling for security_features_present array
                  if (key === 'security_features_present' && Array.isArray(value) && value.length > 0) {
                    return (
                      <div key={key} className="border-b border-gray-100 pb-3 last:border-0">
                        <div className="font-medium text-gray-700 mb-2">Security Features Present</div>
                        <div className="space-y-2">
                          {value.map((feature, idx) => {
                            const feat = feature as { feature_type?: string; location?: string; description?: string };
                            return (
                              <div key={idx} className="bg-white border border-red-200 rounded-lg p-3">
                                <div className="flex items-center gap-2 mb-1">
                                  <Shield className="w-4 h-4 text-red-600" />
                                  <span className="font-semibold text-red-900">
                                    {feat.feature_type?.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())}
                                  </span>
                                </div>
                                {feat.location && (
                                  <div className="text-sm text-gray-700">
                                    <span className="font-medium">Location:</span> {feat.location}
                                  </div>
                                )}
                                {feat.description && (
              ...
              <p className="text-gray-500 italic">No security assessment data</p>
            )}
          </div>
        )}
      </div>
  ...
  // Special renderer for validity_status with disclosure_details array
  const ValidityStatusSection = ({
    data,
  }: {
    data: Record<string, unknown>;
  }) => {
    const isExpanded = expandedSections.has('validity_status');
    const hasData = data && Object.keys(data).length > 0;
    ...
    return (
      <div className="bg-teal-50 rounded-xl shadow-sm border border-teal-200 overflow-hidden">
        <button
          className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors"
          onClick={() => toggleSection('validity_status')}
        >
          <div className="flex items-center gap-3">
            ...
            {hasData ? (
              <div className="space-y-4">
                {Object.entries(data).map(([key, valueObj]) => {
                  ...
                  // Special handling for disclosure_details array
                  if (key === 'disclosure_details' && Array.isArray(value) && value.length > 0) {
                    return (
                      <div key={key} className="border-b border-gray-100 pb-3 last:border-0">
                        <div className="font-medium text-gray-700 mb-2">Disclosure Details</div>
                        <div className="space-y-3">
                          {value.map((disclosure, idx) => {
                            const disc = disclosure as { offense_type?: string; offense_date?: string; conviction_date?: string; details?: string };
                            return (
                              <div key={idx} className="bg-amber-50 border border-amber-300 rounded-lg p-3">
                                ...
                                {disc.offense_type && (
                                  <div className="text-sm mb-1">
                                    <span className="font-medium text-gray-700">Offense Type:</span>{' '}
                                    <span className="text-gray-900">{disc.offense_type}</span>
                                  </div>
                                )}
                                {disc.offense_date && (
                                  <div className="text-sm mb-1">
                                    <span className="font-medium text-gray-700">Offense Date:</span>{' '}
                                    <span className="text-gray-900">{disc.offense_date}</span>
                                  </div>
                                )}
                                {disc.conviction_date && (
                                  <div className="text-sm mb-1">
                                    <span className="font-medium text-gray-700">Conviction Date:</span>{' '}
                                    <span className="text-gray-900">{disc.conviction_date}</span>
                                  </div>
                                )}
                                {disc.details && (
                  ...
                  // Special handling for result_status with color coding
                  if (key === 'result_status') {
                    ...
                    if (status === 'CLEAR') {
                      ...
                      statusIcon = <AlertTriangle className="w-4 h-4 text-amber-700" />;
                    ...
                    return (
                      <div key={key} className="border-b border-gray-100 pb-3 last:border-0">
                        <div className="font-medium text-gray-700 mb-2">Result Status</div>
                        <div className={`flex items-center gap-2 px-3 py-2 ${statusBg} rounded-lg`}>
                          {statusIcon}
              ...
              <p className="text-gray-500 italic">No validity status data</p>
            )}
          </div>
        )}
      </div>
  ...
  if (!webhookData) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="text-2xl font-semibold text-gray-700 mb-4">No overseas police check data found</div>
          <button
            onClick={() => router.back()}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-8">
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold text-gray-900 mb-2">Overseas Police Check</h1>
